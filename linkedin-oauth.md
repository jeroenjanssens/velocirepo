...
Once the `auth` command is implemented:

```bash
velocirepo auth linkedin
```

This will prompt for your Client ID and Secret (first time only), open your browser for authorization, and save the token to `.env`.

### Manual token generation (until the auth command exists)
...
- The redirect URL (`localhost:9876`) does not need to be a running server for the manual flow — you just need to copy the code from the browser's address bar.
